;****************** Lab3.s ***************
; Program written by: Anika Sridhar; Andres Wearden
...
;global variables go here
       ...
       THUMB
       EXPORT EID1
EID1   DCB "aew3364",0  ;replace ABC123 with your EID
       EXPORT EID2
EID2   DCB "as225242",0  ;replace ABC123 with your EID
       ALIGN 4
...
; TExaS_Init sets bus clock at 80 MHz, interrupts, ADC1, TIMER3, TIMER5, and UART0
 ...
 ; Your Initialization goes here
	...
	ORR R1, #0x15
	STR R1, [R0]
	LDR R7, =10000 ; Total duty cycle for breathing
	LDR R2, =3000000 ; Starting duty cycle on period
	LDR R3, =7000000 ; Starting duty cycle off period R2 + R3 = Total duty cycle period
loop  
; main engine goes here
	LDR R0, =0x01
	LDR R0, =GPIO_PORTE_DATA_R
	LDR R1, [R0]
	AND R1, #0x01 ; will loop back if PE0 switch is pressed
	CMP R1, #0x01
	BEQ HighSwitch
Back
	LDR R0, =GPIO_PORTE_DATA_R ; Check to see if PE2 is pressed
	LDR R1, [R0]
	AND R1, #0x04
	CMP R1, #0x04
	BEQ BreathingSwitch
	BIC R1, #0x10
	EOR R1, #0x10 ; toggle LED Pin on
	STR R1, [R0]
	BL DelayOn
	LDR R1, [R0]
	EOR R1, #0x10 ; toggle LED Pin Off
	STR R1, [R0]
	BL DelayOff
	 B    loop
DelayOn
		MOV R4, R2
wait	SUBS R4, R4, #0x01
		BNE wait
		BX LR
DelayOff
		MOV R4, R3
wait2	SUBS R4, R4, #0x01
		BNE wait2
		BX LR
HighSwitch	
	LDR R0, =GPIO_PORTE_DATA_R
	LDR R1, [R0]
	AND R1, #0x01 ; will loop back if switch is pressed
	CMP R1, #0x01; compares to see if switch PE0 pressed, 1
	BEQ HighSwitch; creates a loop and will only exit when PEO is released
	LDR R6, =2000000; adds 20% to the duty cycle
	ADD R2, R6; R2 currently is holding 10%, so now it will hold 30%
	SUBS R3, R6; R3 holds 50%
	LDR R5, =11000000; if it goes over the duty cycle, it will branch to 10%, accounts for the overflow
	CMP R2, R5; checking for overflow, 110%
	BEQ TenPercent
	B Back
TenPercent
	LDR R2, =1000000; 10%
...
BreathingSwitch
	...
	ADD R11, R8, #198
NextAddress
	LDRH R9, [R8]; R9 holds the sin table 
	SUBS R10, R7, R9; R10 holds the off period
	BIC R1, #0x10
	EOR R1, #0x10 ; toggle LED Pin on
	STR R1, [R0]
	BL DelayOnBreathing
...
DelayOnBreathing
		SUBS R9, R9, #0x01
		BNE DelayOnBreathing
		BX LR
DelayOffBreathing
		SUBS R10, R10, #0x01
		BNE DelayOffBreathing
		BX LR
   ALIGN 4   
